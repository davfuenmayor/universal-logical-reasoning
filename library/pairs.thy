...
abbreviation mkPair::"'a \<Rightarrow> 'a \<Rightarrow> Pair('a)" ("\<langle>_,_\<rangle>") 
...
(*Now, observe that*)
lemma "\<langle>x,y\<rangle> \<top> = x" by simp
lemma "\<langle>x,y\<rangle> \<bottom> = y" by simp
...
lemma proj1_char[pair_simps]: "\<pi>\<^sub>1\<langle>x,y\<rangle> = x" by simp
lemma proj2_char[pair_simps]: "\<pi>\<^sub>2\<langle>x,y\<rangle> = y" by simp
lemma mkPair_char[pair_simps]: "\<langle>\<pi>\<^sub>1 P, \<pi>\<^sub>2 P\<rangle> = P" apply(rule ext) by simp

(*Let's now add the useful 'swap' operation on pairs*)
abbreviation swapPair::"Pair('a) \<Rightarrow> Pair('a)" ("'(_')\<Zcat>")
  where "(p)\<Zcat> \<equiv> \<lambda>b. p (\<not>b)"

lemma swapPair_char[pair_simps]: "(\<langle>a, b\<rangle>)\<Zcat> = \<langle>b, a\<rangle>" by auto
lemma swapPair_simp[pair_simps]: "\<langle>\<pi>\<^sub>2 p, \<pi>\<^sub>1 p\<rangle> = (p)\<Zcat>" by auto
